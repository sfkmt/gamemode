...
class GameModeProvider {
    ...
    getTreeItem(element) {
        ...
        item.iconPath = new vscode.ThemeIcon(element.status === 'active' ? 'check' :
            element.status === 'critical' ? 'error' : 'circle-outline', new vscode.ThemeColor(element.status === 'active' ? 'terminal.ansiGreen' :
...
class GameModeStatusBar {
    ...
    updateNetworkStatus(activeNodes, totalNodes) {
...
class MatrixEffect {
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('matrixEffect', 'GAME MODE - Matrix Effect', vscode.ViewColumn.Beside, {
            enableScripts: true,
...
function activate(context) {
    ...
    const activateGameModeCommand = vscode.commands.registerCommand('gameMode.activateGameMode', () => {
        ...
        vscode.window.showInformationMessage('GAME MODE ACTIVATED! Welcome to the Matrix...', 'Show Effects').then((selection) => {
    ...
    vscode.window.showInformationMessage('GAME MODE Extension loaded! Ready to enter the Matrix?', 'Activate GAME MODE').then((selection) => {
